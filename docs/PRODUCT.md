...
| 缓存与存储 | pip、npm、pnpm、Maven、Gradle、vcpkg、Conan 等 | 支持统计、安全迁移和清理 |
...
- **运行时**：发现、安装、切换、修复和卸载 Python、Node.js、Java；
- **工具链**：组合 MSVC/LLVM、Windows SDK、CMake、Ninja 与目标架构；
- **项目环境**：扫描项目需求，安装缺失项，创建锁文件并打开已激活终端；
- **PATH 与变量**：标记来源、重复项、无效项、顺序和同名命令冲突；
- **缓存与存储**：统计 pip/npm 等目录，安全迁移或清理；
- **环境诊断**：解释实际命令来源以及版本不一致的根因；
