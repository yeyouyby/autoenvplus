...
| 类别 | 首批对象 | 说明 |
|---|---|---|
| 运行时 | Python、Node.js、Java、.NET SDK | 支持多版本并存和项目固定 |
...
解析后的精确版本、架构、来源、包哈希算法与校验值写入 schema 2 的 `autoenvplus.lock`。项目配置可以导入 `.python-version`、`.nvmrc`、`.node-version`、`.java-version`、`package.json` 的 `engines` 和 `.NET global.json`。

“打开已激活终端”先把项目选择器解析为当前托管注册表中的精确 Python、Node.js、Java 和 .NET SDK 版本，确认真实可执行文件与对应 Shim 存在，再展示终端宿主、Shell、Shim 目录、请求选择器、精确版本和会话变量。WinUI 在检测到 `wt.exe` 时允许选择 Windows Terminal 或 Windows PowerShell；Windows Terminal 不可用时明确回退到独立 PowerShell，CLI 默认使用 Windows PowerShell。确认后只为新子进程创建环境块并把 Shim 目录置于 PATH 首位；不修改父进程、用户 PATH、全局默认或项目文件。.NET 项目选择写入 `AUTOENVPLUS_DOTNET_VERSION`，`dotnet` Shim 启动精确版本时只为子进程设置对应的 `DOTNET_ROOT` 和 `DOTNET_MULTILEVEL_LOOKUP=0`。CMake 在没有命令 Shim 时仍显示未激活警告。

## 主要页面

- **概览**：统一汇总 Python、Node.js、Java、.NET SDK 的托管数量、全局选择和 PATH 版本，PATH 目录/失效/重复/命令冲突，缓存容量/文件数/最大缓存、受管根磁盘余量、最近项目与最近活动；各摘要可直接进入对应工作页，最近项目可带项目根上下文打开；
- **运行时**：发现、安装、切换、修复和卸载 Python、Node.js、Java 与 .NET SDK；Java 版本线实时来自 Adoptium 官方 GA/LTS 目录，.NET SDK 来自 Microsoft 官方 release metadata；
